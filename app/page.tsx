            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
              {achievements.map((item, i) => {
                ...
                const CardWrapper = ({ children }: { children: React.ReactNode }) =>
                  item.href ? (
                    isExternal ? (
                      <a href={item.href} target="_blank" rel="noopener noreferrer" className="bg-white overflow-hidden shadow-sm group h-full flex flex-col cursor-pointer">
                        {children}
                      </a>
                    ) : (
                      <Link href={item.href} className="bg-white overflow-hidden shadow-sm group h-full flex flex-col cursor-pointer">
                        {children}
                      </Link>
                    )
                  ) : (
